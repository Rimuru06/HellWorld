...
def gameMenu(janela, teclado, mouse, modulo, nivelDificuldade, etapaMenu):
    ...
    while modulo == 1:
        ...
        if etapaMenu == 1:
            ...
            if (mouse.is_over_area((jogar.x, jogar.y),(jogar.x + jogar.width, jogar.y + jogar.height))) and (mouse.is_button_pressed(1)) and (clickTimer > 0.5):
                modulo = 2
                clickTimer = 0
                janela.clear()
            elif (mouse.is_over_area((dificuldade.x, dificuldade.y),(dificuldade.x + dificuldade.width, dificuldade.y + dificuldade.height))) and (mouse.is_button_pressed(1) and (clickTimer > 0.5)):
                etapaMenu = 2
                clickTimer = 0
                janela.clear()
            elif (mouse.is_over_area((sair.x, sair.y),(sair.x + sair.width, sair.y + sair.height))) and (mouse.is_button_pressed(1) and (clickTimer > 0.5)):
                janela.close()
            jogar.draw()
            dificuldade.draw()
            sair.draw()
        if etapaMenu == 2:
            ...
            if (mouse.is_over_area((facil.x, facil.y),(facil.x + facil.width, facil.y + facil.height))) and (mouse.is_button_pressed(1) and (clickTimer > 0.5)):
                janela.clear()
                clickTimer = 0
                modulo = 1
                etapaMenu = 1
                nivelDificuldade = 0.5
            elif (mouse.is_over_area((medio.x, medio.y),(medio.x + medio.width, medio.y + medio.height))) and (mouse.is_button_pressed(1) and (clickTimer > 0.5)):
                janela.clear()
                clickTimer = 0
                modulo = 1
                etapaMenu = 1
                nivelDificuldade = 1
            elif (mouse.is_over_area((dificil.x, sair.y),(dificil.x + dificil.width, dificil.y + dificil.height))) and (mouse.is_button_pressed(1) and (clickTimer > 0.5)):
                janela.clear()
                clickTimer = 0
                modulo = 1
                etapaMenu = 1
